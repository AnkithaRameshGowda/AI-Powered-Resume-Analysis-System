...
export default function AIResumePage() {
  ...
  useEffect(() => {
    if (user?.email) {
      ...
      const oldKeys = ['analysisResults', 'skillRecommendations'];
  ...
  useEffect(() => {
    try {
      const userStorageKey = getUserStorageKey('analysisResults');
      const savedResults = localStorage.getItem(userStorageKey);
      if (savedResults) {
  ...
  const analyzeResume = async () => {
    ...
    try {
      ...
      if (data.success) {
        ...
        // Save complete analysis results to user-specific localStorage
        const userAnalysisKey = getUserStorageKey('analysisResults');
        localStorage.setItem(userAnalysisKey, JSON.stringify(resultsData));
        ...
        localStorage.setItem(userRecommendationsKey, JSON.stringify(data.skillRecommendations || []));
        
        // Debug logging to track ATS feedback flow
        console.log(`DEBUG: ATS feedback saved for user: ${user?.email}`);
        console.log(`DEBUG: Analysis storage key: ${userAnalysisKey}`);
        console.log(`DEBUG: Recommendations storage key: ${userRecommendationsKey}`);
        console.log(`DEBUG: ATS Score: ${data.atsScore}`);
        console.log(`DEBUG: Skill recommendations count: ${data.skillRecommendations?.length || 0}`);
      } else {
