...
export default function JobPage() {
  ...
  useEffect(() => {
    const fetchJobDetails = async () => {
      try {
        ...
        try {
          const appsRes = await api.get('/applications/status');
          const apps = appsRes.data.applications || [];
          const applied = apps.some((a: any) => a.jobTitle === response.data.job.title);
          if (applied) setApplicationStatus('success');
        } catch {}
  ...
  const handleApply = async () => {
    ...
    try {
      ...
      await api.post(`/jobs/${jobId}/apply`, { resumeData, matchScore: 0 });
      ...
      if (errMsg.toLowerCase().includes("already applied")) {
        ...
        toast({ title: "Application Failed", description: errMsg || "There was a problem submitting your application.", variant: "destructive" });
      }
    } finally {
      setApplying(false);
    }
          ...
          <div className="max-w-4xl mx-auto p-4">
            <Card className="p-6 text-center">
              <CardContent>
                ...
                <Button onClick={() => router.push('/home/applicant')}>Back to Jobs</Button>
              </CardContent>
            </Card>
          </div>
          <NavigationBar />
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <Card className="mb-6">
                    ...
                    <CardContent><p className="text-muted-foreground whitespace-pre-line">{job.description}</p></CardContent>
                  </Card>
                  <Card>
                ...
                <div>
                    ...
                    <CardContent>
                      {applicationStatus === 'success' ? (
                        <div className="flex flex-col items-center justify-center p-6 text-center">
                          ...
                          <p className="text-muted-foreground">Your application has been successfully submitted. The recruiter will contact you if your profile matches their requirements.</p>
                        </div>
                      ) : (
                        <div className="flex items-center justify-center border-2 border-dashed border-border rounded-lg p-6">
                          <label className="cursor-pointer">
                            <input type="file" accept=".pdf,.doc,.docx" className="hidden" onChange={handleResumeUpload} disabled={applying} />
                  ...
                  {applicationStatus !== 'success' && (
                    <Button className="w-full" onClick={handleApply} disabled={applying || !resumeFile}>
