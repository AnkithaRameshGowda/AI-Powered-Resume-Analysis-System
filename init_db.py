...
if 'messages' not in db.list_collection_names():
    # Create the messages collection with indexes for efficient querying
    db.create_collection('messages')
    db.messages.create_index([('senderId', 1), ('receiverId', 1), ('timestamp', 1)])
    db.messages.create_index([('receiverId', 1), ('read', 1)])
